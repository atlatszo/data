...
var localization = {
    ...
    "cashIconDefault": {
        "HU": "Átlagos havi Széchenyi-támogatás 2015-2017",
        "EN": "Average monthly Széchenyi-subsidy 2007-2015"
    },
    "humanIconDefault": {
        "HU": "Egy főre jutó havi Széchenyi-támogatás 2015-2017",
        "EN": "Monthly Széchenyi-subsidy per capita 2007-2015"
    },
    "bankIconDefault": {
    ...
    "cashIconSecondary": {
        "HU": "Átlagos Széchenyi-támogatás (millió Ft / hó)",
        "EN": "Average EU-subsidy (€ / month)"
    },
    "humanIconSecondary": {
        "HU": "Egy főre jutó Széchenyi-támogatás (Ft / fő / hó)",
        "EN": "EU-subsidy per capita (€ / month)"
    },
    "bankIconSecondary": {
...
function render_chart(songId, inputData) {
    ...
    if (language === "HU") {
        ...
        var margin = {top: 22, right: 25, bottom: 22, left: 65},
            width = responsivewidth - margin.left - margin.right,
            height = (responsiveHeight-55) - margin.top - margin.bottom;
    }
    else if (language === "EN") {
        if (metric === "DONATION") {
    ...
    if (map.getZoom() <= 9) {
        ...
        if (metric === "DONATION") {
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 45237, 82805, 94770, 147025, 180619, 197000],
                labels = [];
            ...
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 151476, 192046, 239274, 263187, 303578, 393800],
                labels = [];
            if (language === "HU") {
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML +=
        ...
        if (metric === "DONATION") {
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 2082, 2980, 4734, 8005, 11783, 21400],
                labels = [];
            ...
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 78025, 115924, 154308, 205749, 450187, 599999],
                labels = [];
            if (language === "HU") {
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML +=
        ...
        if (metric === "DONATION") {
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 14, 50, 137, 330, 837, 2639],
                labels = [];
            if (language === "HU") {
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML +=
            ...
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 12378, 31617, 54930, 89630, 139710, 242389],
                labels = [];
            if (language === "HU") {
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML +=
    ...
    if (timeSeries != "DEVIATION") {
        ...
        if (map.getZoom() <= 9) {
            ...
            $.ajax({
                dataType: "json",
                async: true,
                url: "resources/szechenyi/megye.json",
                success: function(data) {
            ...
            $.ajax({
                dataType: "json",
                async: true,
                url: "resources/szechenyi/kisterseg.json",
                success: function(data) {
                    map.removeLayer(geojson);
                    geojson = L.geoJson(data, {
                        style: style,
            ...
            $.ajax({
                dataType: "json",
                async: true,
                url: "resources/szechenyi/telepules.json",
                success: function(data) {
                    map.removeLayer(geojson);
                    geojson = L.geoJson(data, {
                        style: style,
